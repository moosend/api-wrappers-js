...
function _ajax(params) {
	...
	if (params.data) xmlhttp.send(JSON.stringify(params.data));
	else xmlhttp.send();
}
...
function _perfCall(urlPath, method, form, callback) {
	...
	if (urlPath.indexOf('?') < 0) urlPath += '?format=json&apikey=' + MoosendAPI.ApiKey;
	else urlPath += '&format=json&apikey=' + MoosendAPI.ApiKey;
