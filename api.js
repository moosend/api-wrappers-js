...
function _perfCall(urlPath, method, form, callback) {
	if (typeof form == 'function') callback = form;

	if (!API.ApiKey) callback('Please set your api key in API.ApiKey');
	if (urlPath.indexOf('?') < 0) urlPath += '?apikey=' + API.ApiKey;
	else urlPath += '&apikey=' + API.ApiKey;

	jQuery.ajax({
		data: form,
		type: method,
	  	url: API.Endpoint + urlPath,
	  	success: function(json) {
	  		if (json.Code != 0) {
	  			callback(_Errors[json.Code]);	
	  		} else {
	  			callback(null, json);
	  		}
	  	},
...
var MoosendAPI = {
	...
	Campaigns : {
		...
		GetCampaigns : function(page, pageSize, callback) {
			...
			_perfCall('/campaigns/' + page + '/' + pageSize + '.json', 'GET', callback);
		},
		...
		GetCampaignStatistics : function(callback) {
		...
		GetActivityByLocation : function(campaignId, callback) {
			if (!campaignId) callback('CampaignId is a required parameter when calling GetActivityByLocation');
			else {
				_perfCall('/campaigns/' + campaignId + '/stats/countries.json', 'GET', callback);
			}
		},
	...
	MailingLists : {
		...
		Delete : function(mailingListId, callback) {
			if (!mailingListId) callback('MailingListID is a required parameter when calling Delete');
				...
				_perfCall('/lists/' + mailingListId + '/delete.json', 'DELETE', createMailingListForm, callback);
			}
		},
		...
		GetMailingListDetails : function(mailingListId, withStatistics, callback) {
			if (!mailingListId) callback('MailingListId is a required parameter when calling GetSubscribersForMailingList');
			else {
				if (typeof withStatistics == 'function') callback = withStatistics;
		...
		GetSubscribersForMailingList : function(mailingListId, status, page, pageSize, callback) {
			if (!mailingListId) callback('MailingListID is a required parameter when calling GetSubscribersForMailingList');
			else if (!status) callback('Status is a required parameter when calling GetSubscribersForMailingList');
			else {
				if (typeof page == 'function') callback = page, page = 1;
				...
				var urlPath = '/lists/' + mailingListId + '/subscribers/' + status + '.json?page=' + page + '&pageSize=' + pageSize;
